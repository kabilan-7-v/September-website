'use client';
import { BaseLight, lightbase, Primary } from "@/constant/color";
            ...
            >
                ...
                <ul className="flex gap-8 items-center">
                    ...
                    <li className="relative group">
                        ...
                        {/* Dropdown Content */}
                        <div className="absolute top-full right-0 mt-3 w-80 origin-top transform transition-all duration-300 ease-in-out scale-95 opacity-0 group-hover:scale-100 group-hover:opacity-100 invisible group-hover:visible">
                            <div className="bg-white rounded-2xl shadow-lg p-6 text-black">
                                {/* PRODUCTS */}
                                <div>
                                    <h3 className="font-bold text-lg mb-2">Products</h3>
                                    <ul className="flex flex-col gap-2">
                                        {["BAP Core", "BAP Code", "BAP Developer Platform", "Pricing", "Contact Sales"].map((item) => (
                                            <li key={item} className="flex justify-between items-center hover:underline cursor-pointer">
                                                <span>{item}</span>
                                ...
                                {/* MODELS */}
                                <div>
                                    <h3 className="font-bold text-lg mb-2">Models</h3>
                                    <ul className="flex flex-col gap-2">
                                        {["Alpha", "Beta", "Gamma"].map((model) => (
                                            <li key={model} className="hover:underline cursor-pointer">
                                                {model}
                                            </li>
                                        ))}
                                    </ul>
                                ...
                                {/* LOG IN */}
                                <div>
                                    <h3 className="font-bold text-lg mb-2">Log in</h3>
                                    <ul className="flex flex-col gap-2">
                                        {["BAP.ai", "BAP Console"].map((login) => (
                                            <li key={login} className="flex justify-between items-center hover:underline cursor-pointer">
                                                <span>{login}</span>
                                                <ExternalLink size={16} />
                                            </li>
                                        ))}
